...
#include <string>
ofEvent<vector<bool> > ParallelCoordinates::brushedSelection = ofEvent<vector<bool> >();
...
void ParallelCoordinates::setup(TableReader table,int width, int height, int x, int y){
    ...
    for(int i=0;i<data.size();i++){
        ...
        std::tuple<float,float,float,float> foo2 (px,y+400, px+20, y+400);
        bar.push_back(foo);
        bar.push_back(foo2);
        px=px+space;
    }
...
void ParallelCoordinates::draw(){
    s.setup(get_vector(click1), get_vector(click2), 250, 250, 200, 650,names[click1], names[click2]);
    s.draw();
    points.clear();
    int limit=table.getAttributesize();
    int space= width/limit+10;
    ...
    for (int cols=0; cols<limit; cols++) {
        ...
        for (int j=0; j<data[cols].size(); j++) {
            ...
            if (data[cols][j].isNum==1) {
                ...
                y1 = ofMap(data[cols][j].value, min, max, 0, 400);
            }
            else{
                std::tuple<float,float> foo=r[cols];
                this-> max=std::get<0>(foo);
                this-> min=std::get<1>(foo);
                y1 = ofMap(convert(data[cols][j].s), min, max, 0, 400);
            }
            if(flip[cols]==true){
                y1=y1;
            }
            ...
            if(cols<limit-1){
                if (data[cols+1][j].isNum==1) {
                    ...
                    this-> max=std::get<0>(foo);
                    this-> min=std::get<1>(foo);
                    y2 = ofMap(data[cols+1][j].value, min, max, 0, 400);
                }
                else{
                    std::tuple<float,float> foo=r[cols+1];
                    this-> max=std::get<0>(foo);
                    this-> min=std::get<1>(foo);
                    y2 = ofMap(convert(data[cols+1][j].s), min, max, 0, 400);
                }
        ...
        if(data[cols][0].isNum){
            ...
            if(flip[cols]){
                ...
                myFont.drawString(to_string(max), -10, 420);
            }
            else{
                myFont.drawString(to_string(max), -10, -10);
                myFont.drawString(to_string(min), -10, 420);
            }
        ...
        if(bounds.getWidth()>space){
            myFont2.drawString(split(names[cols]), -10, 450);
        }
        else{
            myFont2.drawString(names[cols], -10, 450);
...
void ParallelCoordinates::setupString(int space, int cols, int n, int n2){
    if(flip[cols]){
        ...
        if(bounds.getWidth()>space){
            myFont2.drawString(split(data[cols][n2].s), -10, 420);
        }
        else{
            myFont2.drawString(data[cols][n2].s, -10, 420);
        ...
        if(bounds.getWidth()>space){
            myFont2.drawString(split(data[cols][n].s), -10, 420);
        }
        else{
            myFont2.drawString(data[cols][n].s, -10, 420);
...
bool ParallelCoordinates::inside2(float _x, float _y ){
    int limit=table.getAttributesize();
    int space= width/limit+10;
    int px=55;
    int py=90;
    ...
    for (int i=0; i<data.size(); i++) {
        if(ofInRange(_x,px,py)&& ofInRange(_y,80,480) ){
            if(sw==false){
                if(tempx==clickx && tempy==clicky){
                    click1=i;
                    sw=true;
                }
                else{
...
std::vector<float> ParallelCoordinates::get_vector(int which_column){
    if(which_column>table.countAtt){
        cout<<"error"<<endl;
    }
    if (data[which_column][0].isNum) {
        std::vector<float> temp;
        for (int i=0; i<data[which_column].size(); i++) {
            temp.push_back(data[which_column][i].value);
        }
        return temp;
    }
    else{
        std::vector<float> temp;
        for (int i=0; i< data[which_column].size(); i++) {
            float f=convert(data[which_column][i].s);
            temp.push_back(f);
